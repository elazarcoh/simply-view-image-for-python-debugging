...
pub fn calculate_camera_after_wheel_zoom(
    ...
    let clip_coordinates = get_clip_space_mouse_position(event.clone().dyn_into().unwrap(), canvas);
    ...
    let view_projection_matrix_inv =
        camera::calculate_view_projection(&canvas_size, &view_size, camera).inverse();
    let pre_zoom_position = (view_projection_matrix_inv * clip_coordinates.to_hom()).xy();
    ...
    let post_zoom_position = (view_projection_matrix_inv * clip_coordinates.to_hom()).xy();
