...
suite('Extension Activation Test Suite', () => {
  ...
  it('extension commands should be registered', async () => {
  ...
  it('extension configuration should be available', () => {
  ...
  it('extension should register tree view provider', async () => {
    ...
    const treeView = vscode.window.createTreeView('pythonDebugImageWatch', {
      treeDataProvider: {
        getChildren: () => [],
        getTreeItem: (_element: any) => new vscode.TreeItem('test'),
      },
    });
